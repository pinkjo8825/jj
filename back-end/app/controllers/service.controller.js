...
function create(req, res) {
  ...
  const serviceObj = new Service({
    ...
    additionServiceId: req.body.additionServiceId,
  });
  Service.create(serviceObj, (err, data) => {
    if (err) {
      res.status(500).send({
...
function update(req, res) {
  ...
  Service.update(req.params.id, data, (err, result) => {
    if (err) {
...
function del(req, res) {
  Service.del(req.params.id, (err, result) => {
    if (err) {
      if (err.kind == "not_found") {
        res.status(401).send({
          message: "Not found id: " + req.params.id,
